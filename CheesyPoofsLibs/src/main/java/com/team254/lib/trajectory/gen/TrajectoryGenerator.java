...
public class TrajectoryGenerator
{
     ...
     *
     * @param config
     *            Definition of constraints and sampling rate (WARNING: Some may
     *            be ignored)
     * @param strategy
            ...
            double goal_heading)
    {
        ...
        if (strategy == StepStrategy)
        {
            ...
            traj = secondOrderFilter(1, 1, config.dt, config.max_vel, config.max_vel, impulse, time, TrapezoidalIntegration);
        ...
        else if (strategy == TrapezoidalStrategy)
        {
            ...
            traj = secondOrderFilter(f1_length, 1, config.dt, start_vel, adjusted_max_vel, impulse, time, TrapezoidalIntegration);
        ...
        else if (strategy == SCurvesStrategy)
        {
            ...
            double adjusted_max_vel = Math.min(config.max_vel,
                    (-config.max_acc * config.max_acc + Math.sqrt(config.max_acc * config.max_acc * config.max_acc * config.max_acc
                            + 4 * config.max_jerk * config.max_jerk * config.max_acc * goal_pos)) / (2 * config.max_jerk));
            ...
            traj = secondOrderFilter(f1_length, f2_length, config.dt, 0, adjusted_max_vel, impulse, time, TrapezoidalIntegration);
        ...
        {
            traj.getSegment(i).heading = start_heading
                    + total_heading_change * (traj.getSegment(i).pos) / traj.getSegment(traj.getNumSegments() - 1).pos;
        }
            ...
            int length, IntegrationMethod integration)
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            traj.getSegment(i).vel = f2 / f2_length * max_vel;

            if (integration == RectangularIntegration)
            {
                traj.getSegment(i).pos = traj.getSegment(i).vel * dt + last.pos;
            }
            else if (integration == TrapezoidalIntegration)
            {
                traj.getSegment(i).pos = (last.vel + traj.getSegment(i).vel) / 2.0 * dt + last.pos;
            }
            traj.getSegment(i).x = traj.getSegment(i).pos;
            traj.getSegment(i).y = 0;

            // Acceleration and jerk are the differences in velocity and
            // acceleration, respectively.
            traj.getSegment(i).acc = (traj.getSegment(i).vel - last.vel) / dt;
            traj.getSegment(i).jerk = (traj.getSegment(i).acc - last.acc) / dt;
            traj.getSegment(i).dt = dt;

            last = traj.getSegment(i);
        }
